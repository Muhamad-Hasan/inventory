...
router.post("/" , async(req , res)=>{
    ...
    try{
        ...
        if(!value.error){
            ...
            let customer = await Customer.find({c_id : data.c_id});
            if(customer && customer.length > 0){
                console.log("next" , customer);
                return res.status(400).json("Customer already exsists on this Id")
            }else{
...
router.get("/" , async(req , res)=>{
    ...
    try{
        ...
        if(!value.error){
            if(data.name){
                let customer = await Customer.find({name : {$regex : data.name} , deleted : false}).sort({_id : -1});
                res.status(200).json(customer);
            }else{
                let customer = await Customer.find({deleted : false}).sort({_id : -1});
                res.status(200).json(customer);
            }
        }
    }catch(err){
...
router.get("/:c_id" , async(req , res)=>{
    try{
        if(req.params.c_id){
            let customer = await Customer.findById(req.params.c_id);
            if(customer){
                res.status(200).json(customer);
            }
        }    
...
router.get("/:id" , async(req , res)=>{
    ...
    try{
        if(data.id){
            let raw = await Customer.findById(data.id)
            res.status(200).json(raw)
...
router.put("/:id" , async(req , res)=>{
    ...
    try{
        if(data.id){
            let raw = await Customer.findByIdAndUpdate(data.id , {
                $set : req.body
            } ,{new : true})
            res.status(200).json(raw)
